    ...
    if (result == 0) {
        ...
        if (ifdef == 1) {
            ...
            if (check_type(query_type, functiontype) == 0) {
                error_s(19, cur->column, IDnode->string_contant, NULL); // type不同报错
                flag = 4;
            }
        }
    }
    ...
    if (strcmp(tempnode0->name, "TYPE") == 0) {
        ...
        //     printf("In OptTag\n");
            ...
            */    
            ...
            type->u.structure_.name = (char *)malloc(sizeof(char)*32);
            strcpy(type->u.structure_.name, struct_name); // while (赋值struct name;用于返回给上层;同时struct name作为hash值将填到struct hash表里面;
